...
function defenseSoldat() {
    ...
    if (alien0.pv <= 0) {
        setTimeout(function () {
            document.getElementById("eventRead").innerHTML = "Vous avez résisté à l'assault des aliens !<br>Cependant vous avez perdu des soldats dans la bataille... ";
        }, 1000);
        setTimeout(function () {
            updateRessource();
        }, 4000);
...
function defenseCharDecombat() {
    if (charDeCombat0.nombre > 0) {
        ...
        if (alien0.pv <= 0) {
            setTimeout(function () {
                document.getElementById("eventRead").innerHTML = "Vos chars ont résisté à l'assault des aliens !<br> ";
            }, 1000);
            setTimeout(function () {
                updateRessource();
            }, 2000);
...
function defenseTourelleAuto() {
    if (tourelleAuto0.nombre > 0) {
        ...
        while (alien0.pv > 0) {
            ...
            if (alien0.pv <= 0) {
                setTimeout(function () {
                    document.getElementById("eventRead").innerHTML = "Vos tourelles Autos ont résisté à l'assault des aliens !<br> ";
                }, 1000);
                setTimeout(function () {
                    updateRessource();
                }, 2000);
...
function quest17N() {
    ...
    if (pop > randomEventResultat) {
        pop -= randomEventResultat
    } else {
        if (popOr > randomEventResultat) {
            popOr -= randomEventResultat
        } else {
            if (popFer > randomEventResultat) {
                popFer -= randomEventResultat
            } else {
                if (popGaz > randomEventResultat) {
                    popGaz -= randomEventResulta
                } else {
                    if (soldat0.nombre > randomEventResultat) {
                        soldat0.nombre -= randomEventResultat
                    }
                }
            }
        }
